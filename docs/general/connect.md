...
You'll need to change the CAN protocol in the BMS settings to "Victron". If your BMS has a screen, the easiest way to do so might be on the screen, on the UP series go to PaskSet -> CANBus -> choose Can-Victron. Otherwise, use the correct software for your BMS series. For UP series that would be [JBD-ES-UP](https://diysolarforum.com/resources/jbd-smart-up16s-series-bms-used-by-eco-worthy-48v-rack-battery.477/) (protocol dropdowns are on the main screen. If "Set" buttons are inactive, click Account -> User Login in the menu and enter the password from the link). For other series it might be [JBDTools](https://jbdtools.oss-cn-beijing.aliyuncs.com/packfile/JBDTools.zip) (look for Protocol Settings on the Other tab). Some Bluetooth apps mentioned in the links above might also work, but the PC software appears to be more well-maintained.
